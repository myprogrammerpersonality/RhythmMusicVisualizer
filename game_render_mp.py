...
def render_frames(start, end, radii, width, height, output_dir):
    pygame.init()
    ...
    for i in range(start, end):
        ...
        frame_filename = os.path.join(output_dir, f"frame_{i:05d}.png")
...
def main(args):
    ...
    for i in range(num_processes):
        ...
        p = mp.Process(target=render_frames, args=(start, end, new_vals_compressed, args.width, args.height, output_dir))
        processes.append(p)
        p.start()
